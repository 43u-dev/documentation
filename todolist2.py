...
class TodolistApp(tk.Tk):
    ...
    def load_tasks(self):
        ...
        for idx, task in enumerate(self.cursor.fetchall()):
            ...
            if task[-1] == 1:
                ...
                priority = task[3].lower()
                if priority == "Grande":
                    self.task_list.itemconfig(idx, fg="red")
                elif priority == "moyenne":
                    self.task_list.itemconfig(idx, fg="green")
                elif priority == "basse":
                    self.task_list.itemconfig(idx, fg="orange")
    ...
    def filter_tasks(self):
        ...
        for task in self.cursor.fetchall():
            self.insert_task_in_listbox(task)
